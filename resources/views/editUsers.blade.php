...
@section('content')
    <div>
    <form action="{{ route('admin.users.update', $user) }}" class="signin-form" method="post">
        @csrf
        @method('PUT')
                ...
                <div class="form-group">
                  <label class="form-control-placeholder" >Privilegios</label>
                  <select class="custom-select" name="id_privilege">
                    <option value=0>Usuario</option>
                    <option value=1>Aprendiz</option>
                    @if( Auth::user()->id_privilege > 2)
                      <option value=2>Maestro</option>
                    @endif
                    @if( Auth::user()->id_privilege == 4 || Auth::user()->id == $user->id)
                      <option value=3>Administrador</option>
                    @endif
                    @if( Auth::user()->id_privilege == 4)         
                      <option value=4>Super Administrador</option>
                    @endif
                  </select>
                </div>
                <div class="form-group">
                  <label class="form-control-placeholder" >Género</label>
                ...
                <div class="form-group">
                  ...
                  <select class="custom-select" name="id_country">
                    ...
                    <option value=139 selected>Peru</option>
...
@section('js')
    <script>
        jQuery(document).ready(function($){
            $('select[name=id_country]').find('option[value={{$user->id_country}}]').attr('selected','selected');
            $('select[name=gender]').find('option[value={{$user->gender}}]').attr('selected','selected');
            $('select[name=id_privilege]').find('option[value={{$user->id_privilege}}]').attr('selected','selected');
